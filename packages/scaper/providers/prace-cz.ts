...
function parseListingsFromHtml(html: string): Listing[] {
  ...
  $("li.search-result__advert").each((_, li) => {
    ...
    const company = clean(
      $li
        .find(".search-result__advert__box__item--company")
        .first()
        .text()
        .replace("•", ""),
    );
    const location = clean(
      $li
        .find(".search-result__advert__box__item--location strong")
        .first()
        .text(),
    );
    const status = clean(
      $li
        .find(".text-label--important, .search-result__advert__supermax")
        .first()
        .text(),
    );
